...
function write_state() {
  INDEX=$1
  echo "S$INDEX     \"--s$INDEX \"  c   (0,1,2,3,4,5) | as.numerci(NumStates)>$INDEX " >> ${TXT_FILE}
...
function write_connection() {
  STATE=$1
  CONNECTION=$2
  echo "N${STATE}x$CONNECTION  \"--n${STATE}x$CONNECTION \" c   (0,1,2,3) | as.numeric(NumConnections$STATE)>$CONNECTION " >> ${TXT_FILE}
  echo "C${STATE}x$CONNECTION  \"--c${STATE}x$CONNECTION \" c   (0,1,2,3,4,5) | as.numeric(NumConnections$STATE)>$CONNECTION " >> ${TXT_FILE}
  echo "P${STATE}x$CONNECTION  \"--p${STATE}x$CONNECTION \" r   (0,1) | as.numeric(C${STATE}x$CONNECTION) %in% c(0,2,3,5) " >> ${TXT_FILE}
  echo "B${STATE}x$CONNECTION  \"--p${STATE}x$CONNECTION \" i   (1,10) | as.numeric(C${STATE}x$CONNECTION)==1 " >> ${TXT_FILE}
  echo "W${STATE}x$CONNECTION  \"--w${STATE}x$CONNECTION \" r   (0,20) | as.numeric(C${STATE}x$CONNECTION)==1 " >> ${TXT_FILE}
